...
function render_a_form(idx) {
    ...
    newElement += '<img class="off" id="onOff' + (idx * 2 + 1) + '" src="/assets/img/helpoff.png" style="float: left; width: 30px">';
    newElement += '<p class="explainText" id="explain' + (idx * 2 + 1) + '"> نام و نام خانوادگی خود را بر اساس کارت ملی و یا شناسنامه درج نمایید</p>';
    ...
    newElement += '<img class="off" id="onOff' + (idx * 2 + 2) + '" src="/assets/img/helpoff.png" style="float: left; width: 30px">';
    newElement += '<p class="explainText" id="explain' + (idx * 2 + 2)  + '">لطفاً پس از وارد کردن نام و نام خانوادگی به همراه کد ملی دکمه سنجش اصالت را فشار دهید. سایر اطلاعات هویتی توسط سامانه شاهکار پر خواهد گردید</p>';
    newElement += '</div>';
    ...
    $("#onOff" + (idx * 2 + 1)).on('click', function () {
        toggleOnOff(idx * 2 + 1);
    });

    $("#onOff" + (idx * 2 + 2)).on('click', function () {
        toggleOnOff(idx * 2 + 2);
    });
}
...
function doSubmit() {
    ...
    if(!hasErr)
        document.location.href = '/farayand/1/2/' + instId;
}